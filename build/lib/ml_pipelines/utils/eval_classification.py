...
def make_precision_recall_curve(y_true, y_probas, class_names, save_folder, figsize):
    """
    Plots a beautiful precision-recall curve for multiclass classification.
    ...
    for i in range(len(class_names)):
        precision, recall, _ = precision_recall_curve(y_true_bin[:, i], y_probas[:, i])
        auc_score = average_precision_score(y_true_bin[:, i], y_probas[:, i])
        plt.plot(
            recall, precision, lw=2, label=f"{class_names[i]} (AUC = {auc_score:.3f})"
        )
...
def make_roc_curve(y_true, y_probas, class_names, save_folder, figsize):
    """
    Plots a beautiful ROC curve for multiclass classification.
